...
class MemoryEvolver:
    ...
    def __init__(self, threshold: int = 5, graph_svc: GraphService | None = None, redis_svc: RedisService | None = None):
    ...
    async def evolve_subtree(self, root_id: str, BLOAT_THRESHOLD: int = 20):
        ...
        candidates = await self.graph.find_duplicate_candidates(
            node_ids, threshold=0.88, root_id=root_id
        )
        ...
        for node_a, node_b in candidates:
                ...
                messages=[
                    {"role": "system", "content": EVOLVER_SYSTEM_PROMPT},
                    {
                        ...
                        "content": EVOLVER_USER_TEMPLATE.format(
                            cat_a=root_id, desc_a=node_a.description, cont_a=node_a.content, time_a=node_a.updated_at,
                            cat_b=root_id, desc_b=node_b.description, cont_b=node_b.content, time_b=node_b.updated_at,
                        ),
                    },
                ],
            ...
            if decision.should_merge:
                ...
                merged_node = KnowledgeNode(
                    ...
                    worth_of_learning=decision.merged_worth_of_learning or max(node_a.worth_of_learning, node_b.worth_of_learning),
    ...
    async def split_bloated_subtree(self, root_id: str, nodes: list[KnowledgeNode]):
        ...
        try:
            clustering = await invoke_llm_structured(
                messages=[
                    ...
                    {"role": "user", "content": AUTO_CLUSTERING_USER_TEMPLATE.format(cat_name=cat_name, nodes_info=nodes_info)}
                ],
        ...
        for cluster in clustering.clusters:
            ...
            await self.graph.create_category_node(Category(
                id=new_cat_id, name=cluster.name, summary=cluster.summary, level=cat_level + 1
            ))
            
            await self.graph.create_relationship(Relationship(
                source_id=new_cat_id, target_id=root_id,
            ...
            for nid in cluster.node_ids:
                ...
                async with self.graph.driver.session() as session:
                    await session.run("MATCH (n:Knowledge {id: $nid})-[r:BELONGS_TO]->(:Category {id: $cid}) DELETE r", nid=nid, cid=root_id)
                
                await self.graph.create_relationship(Relationship(
                    source_id=nid, target_id=new_cat_id,
    ...
    async def re_summarize_subtree(self, root_id: str):
        ...
        async with self.graph.driver.session() as session:
            parent_res = await session.run(
                "MATCH (c:Category {id: $id})-[:SUB_CATEGORY_OF]->(p:Category) RETURN p.id as id",
                id=root_id
            )
