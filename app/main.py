...
async def lifespan(app: FastAPI):
    """Handle startup and shutdown for Connect."""
...
async def create_topic(request: CreateTopicRequest):
    ...
    async with graph_service.driver.session() as s:
        ...
        rec = await s.single()
        if rec:
            parent_level = rec["level"]

    # Create category node in Neo4j
    await graph_service.create_category_node(
        Category(
            id=session_id,
    ...
    metadata = {"topic_name": request.topic_name, "parent_category_id": parent_category_id}
    ...
    return CreateTopicResponse(session_id=session_id, category_id=session_id)
...
async def chat(request: ChatRequest):
    ...
    return ChatResponse(session_id=session_id, response=llm_response, retrieved_node_ids=retrieved_ids)
...
async def chat_stream(request: ChatRequest):
    ...
    # Retrieve Context (Hybrid Memory)
    memory_context, retrieved_ids = await memory_retriever.get_relevant_context(
        session_id, request.message, category_ids=request.category_ids
    )
    logger.info(f"Memory context retrieved: {memory_context[:200]}...")
...
async def extract_context(session_id: str, background_tasks: BackgroundTasks):
    ...
    Requires a real session ID (e.g. session_xxx or mcp_quick_xxx), not a category ID (cat_xxx).
    """
    if session_id.startswith(CATEGORY_ID_PREFIX):
        logger.warning(f"[extract] Rejected category ID used as session_id: {session_id}")
        return (
            {"detail": "Use a chat session ID, not a category ID. Select a chat from the sidebar and click Extract."},
            400,
        )
...
async def refresh_memory(session_id: str, background_tasks: BackgroundTasks):
    ...
    Requires a real session ID, not a category ID (cat_xxx).
    """
    if session_id.startswith(CATEGORY_ID_PREFIX):
        return (
            {"detail": "Use a chat session ID, not a category ID. Select a chat from the sidebar."},
            400,
        )
