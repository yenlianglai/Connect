...
class VerticalRelationshipType(StrEnum):
...
class HorizontalRelationshipType(StrEnum):
...
class KnowledgeNode(BaseModel):
    ...
    content: str = Field(
        ...,
        description="Technical details: WHAT problem solved, WHY this approach, and HOW to do it. For facts, this is the fact text.",
    )
    description: str = Field(..., description="LLM's concise summary. For facts, this can be the same as content.")
    tags: list[str] = Field(
        default_factory=list,
        description="Keywords for horizontal retrieval. For facts, includes fact_type (identity/preference/habit).",
    )
...
class Relationship(BaseModel):
    ...
    relationship_type: VerticalRelationshipType | HorizontalRelationshipType = Field(
...
class ExtractionResult(BaseModel):
    ...
    knowledge_nodes: list[KnowledgeNode] = Field(
        default_factory=list, description="Includes both learnable knowledge and user facts"
    )
