...
from app.models.nodes import Category, VerticalRelationshipType, Relationship
...
async def test_end_to_end_memory_lifecycle(infra):
    ...
    cat_se = Category(id="cat_se", name="Software Engineering", summary="Software engineering principles and practices", level=1)
    await graph_svc.create_category_node(cat_se)
    await graph_svc.create_relationship(Relationship(
        source_id="cat_se", target_id="cat_root", 
        relationship_type=VerticalRelationshipType.SUB_CATEGORY_OF
    ))

    # 4. Prepare Message Data
    with open("tests/data/messages.json", "r") as f:
        data = json.load(f)
    ...
    for msg in batch_1:
        ...
        if "Updated room membership" in text: continue
    ...
    with patch("app.services.extractor.graph_service", graph_svc), \
        ...
        messages_for_llm = [
            {"role": "system", "content": f"You are a helpful assistant. Use the following memory context to answer correctly: {memory_context}"},
            *history,
        ...
        assert any(word.lower() in response.lower() for word in ["Redis", "Memory", "Memorystore", "短期記憶", "NOSQL", "google sheet"])
